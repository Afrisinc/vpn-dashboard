...
export function OrganizationDetailsSheet({
  ...
  const [confirmRemove, setConfirmRemove] = useState<{ memberId: string; memberEmail: string } | null>(null);

  const { data: membersData, isLoading: membersLoading } = usePlatformOrganizationMembers(
        ...
        <SheetHeader className="flex-shrink-0">
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <SheetDescription className="truncate">{organization.legal_name || "N/A"}</SheetDescription>
            </div>
            <Badge variant={statusVariant(organization.status)} className="flex-shrink-0">
        ...
        >
          <TabsList className="grid w-full grid-cols-2 flex-shrink-0">
            ...
            <TabsTrigger value="members" className="gap-2">
              ...
              {membersData && <span className="ml-1 text-xs">({membersData.members.length})</span>}
            </TabsTrigger>
          </TabsList>
                  ...
                  <CardContent className="space-y-3">
                    <div className="space-y-2 text-sm">
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <span className="text-right break-words">{organization.name}</span>
                      </div>

                      <div className="flex justify-between items-center">
                        <span className="text-muted-foreground">Legal Name</span>
                        <span className="text-right">{organization.legal_name || "—"}</span>
                      </div>

                      {organization.country && (
                        <div className="flex justify-between items-center">
                          <span className="text-muted-foreground">Country</span>
                          <span className="text-right">{organization.country}</span>
                        </div>
                      )}
                      ...
                      {organization.tax_id && (
                        <div className="flex justify-between items-center">
                          ...
                          <span className="text-right font-mono text-xs">{organization.tax_id}</span>
                        </div>
                      )}
                      ...
                      {organization.org_email && (
                        <div className="flex justify-between items-center">
                          ...
                          <span className="text-right break-words">{organization.org_email}</span>
                        </div>
                      )}
                      ...
                      {organization.org_phone && (
                        <div className="flex justify-between items-center">
                          ...
                          <span className="text-right">{organization.org_phone}</span>
                        </div>
                      )}
                      ...
                      {organization.location && (
                        <div className="flex justify-between items-center">
                          ...
                          <span className="text-right">{organization.location}</span>
                        </div>
                      )}
                  ...
                  <Card>
                    <CardContent className="pt-6">
                      <div className="text-2xl font-bold">
                        {membersData?.members.filter((m) => m.role === "OWNER").length || 0}
                      </div>
